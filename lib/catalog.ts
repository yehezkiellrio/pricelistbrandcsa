...
import type { CatalogSnapshot, Product, ProductSku } from '@/lib/types';
...
export async function fetchCatalogSnapshot(
  ...
  const [brandResult, categoryResult, productResult, skuResult, imageResult, priceResult] = await Promise.all([
    ...
    client.from('product_skus').select('id, product_id, code, name, description, sort_order').order('sort_order').order('id'),
    client.from('product_images').select('id, product_id, sku_id, storage_path, is_primary, sort_order').order('is_primary', { ascending: false }).order('sort_order').order('id'),
  ...
  const skusByProduct = new Map<number, DbRow[]>();
  for (const row of skuRows) {
    const productId = Number(row.product_id);
    skusByProduct.set(productId, [...(skusByProduct.get(productId) ?? []), row]);
  }
  ...
  const products: Product[] = priceRows.flatMap((priceRow) => {
    ...
    const skuOptions: ProductSku[] = (skusByProduct.get(productId) ?? []).map((row) => ({
      id: Number(row.id),
      code: textOrNull(row.code),
      name: textOrNull(row.name) ?? 'Tanpa nama',
      description: textOrNull(row.description),
      image_paths: imagesByTarget.get(`sku:${Number(row.id)}`) ?? [],
    }));
    const imagePaths = [...new Set([
      ...(imagesByTarget.get(`product:${productId}`) ?? []),
      ...(skuId === null
        ? skuOptions.flatMap((option) => option.image_paths)
        : imagesByTarget.get(`sku:${skuId}`) ?? []),
    ])];
