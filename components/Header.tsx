...
function NavIcon({ name }: { name: string }) {
  ...
  if (name === 'admin') return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3.2 20 6v5.7c0 4.5-3.1 7.9-8 9.1-4.9-1.2-8-4.6-8-9.1V6Z"/><path d="M9.2 11.2V9.8a2.8 2.8 0 0 1 5.6 0v1.4M8.4 11.2h7.2v5.2H8.4Z"/></svg>;
    ...
    <>
      <header className="topbar">
        <div className="shell nav-wrap">
          <a href="/" className="brand-lockup" aria-label="CSA Master Pricelist">
            <Image src="/csa-logo.svg" alt="PT Catur Sentosa Adiprana Tbk" width={184} height={58} priority />
          </a>
          <nav className="desktop-nav" aria-label="Navigasi utama">
            {items.map((item) => <a key={item.href} className={active(item.href) ? 'active' : ''} href={item.href}>{item.label}</a>)}
          </nav>
        </div>
      </header>
      <nav className="mobile-nav" aria-label="Navigasi utama">
        {items.map((item) => <a key={item.href} className={active(item.href) ? 'active' : ''} href={item.href}><NavIcon name={item.icon}/><span>{item.label}</span></a>)}
      </nav>
    </>
  );
}
