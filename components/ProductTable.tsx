...
export default function ProductTable({ products, brands, categories }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((product) => {
      const haystack = [product.brand, product.category, product.name, product.sku, product.minimum_order, product.discount]
  ...
  return (
    <>
      ...
      <div className="product-cards">
        {filtered.map((product) => <a className="product-card" href={`/master-harga/${product.id}`} key={product.id}><div><span>{product.brand || product.category || 'Produk'}{product.promo_valid_until ? ' · Promo' : ''}</span><strong>{product.name || 'Tanpa nama'}</strong><small>{[product.sku, product.unit, product.minimum_order].filter(Boolean).join(' · ')}</small></div><div className="card-price"><strong>{formatRupiah(effectivePrice(product))}</strong><span>›</span></div></a>)}
      </div>
    </>
  );
}
