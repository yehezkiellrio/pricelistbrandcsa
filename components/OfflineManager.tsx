...
export default function OfflineManager() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('csa-offline-version-v5') === String(version)) setState('ready');
    else setState('idle');
    ...
    const receiveMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'OFFLINE_READY') {
        localStorage.setItem('csa-offline-version-v5', String(event.data.version ?? version));
